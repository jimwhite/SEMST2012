...
### negations in gold
  ...
  for ($i=0;$i<=$max_neg_cols_g;$i++){
   ...
   for ($z=0;$z<=$max_negs_g;$z++){
      if (!(defined(@{$neg_words_g[$z]}))){
	@{$neg_words_g[$z]} = ();
      }
     if (!(defined(@{$neg_tokens_g[$z]}))){
	@{$neg_tokens_g[$z]} = ();
      }
...
### negations in system
  ...
  for ($z=0;$z<=$max_negs_p;$z++){
      if (!(defined(@{$neg_words_p[$z]}))){
	@{$neg_words_p[$z]} = ();
      }
     if (!(defined(@{$neg_tokens_p[$z]}))){
	@{$neg_tokens_p[$z]} = ();
      }
	...
	$found =0;
	  ...
	  for ($y=0;$y<=$max_negated_tokens_g;$y++){
	    for ($x=0;$x<=$max_negated_tokens_p;$x++){
	      ...
	      if ($negated_tokens_g[$i][$y] == $negated_tokens_p[$z][$x]){
		$found = 1;
		last;
